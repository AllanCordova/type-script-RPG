...
describe("Enemy (extending Persons)", () => {
    ...
    it("should create an enemy with correct initial stats", () => {
        const goblin = new Enemy_1.Enemy("Goblin", 50, 10, 5, "");
    ...
    it("should throw InvalidStatError for non-positive initial stats", () => {
        expect(() => new Enemy_1.Enemy("Fantasma", 0, 10, 5, "")).toThrow(InvalidStatError_1.InvalidStatError);
        expect(() => new Enemy_1.Enemy("Fantasma", 50, 0, 5, "")).toThrow(InvalidStatError_1.InvalidStatError);
    });
    // Teste de métodos
    describe("takeDamage", () => {
        it("should reduce HP correctly when taking damage", () => {
            const orc = new Enemy_1.Enemy("Orc", 100, 15, 8, "");
            orc.takeDamage(20);
            expect(orc.hp).toBe(80);
        });
        it("should not let HP go below zero", () => {
            const orc = new Enemy_1.Enemy("Orc", 20, 15, 8, "");
            orc.takeDamage(50);
    ...
    describe("heal", () => {
        it("should restore HP correctly", () => {
            const troll = new Enemy_1.Enemy("Troll", 100, 20, 12, "");
    ...
    describe("attack", () => {
        it("should make the target take the correct amount of damage", () => {
            const knight = new Player_1.Player("Sir Lancelot", 100, 20, 15, " ");
            const dragon = new Boss_1.Boss("Smaug", 200, 25, 10, "");
